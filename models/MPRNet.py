...
class Encoder(nn.Module):
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        enc1 = self.encoder_level1(x) #相当于论文Fig3中的图C中的Next Encoder中的提取到的特征。
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc1 = enc1 + self.csff_enc1(encoder_outs[0]) + self.csff_dec1(decoder_outs[0]) #分别对上一stage，encoder输出和decoder输出
...
class ORSNet(nn.Module):
    ...
    def forward(self, x, encoder_outs, decoder_outs):
        x = self.orb1(x) #使用ORB提取特征，
        x = x + self.conv_enc1(encoder_outs[0]) + self.conv_dec1(decoder_outs[0]) #执行CSFF操作，执行的是Fig3中的（d）

        x = self.orb2(x)
        x = x + self.conv_enc2(self.up_enc1(encoder_outs[1])) + self.conv_dec2(self.up_dec1(decoder_outs[1])) #将特征图放大两倍再使用CSFF融合

        x = self.orb3(x)
        x = x + self.conv_enc3(self.up_enc2(encoder_outs[2])) + self.conv_dec3(self.up_dec2(decoder_outs[2])) #将特征放大4倍再使用CSFF融合。
