    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(
                    *[ScaleAwareGatedBlock(chan) for _ in range(num)]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[ScaleAwareGatedBlock(chan) for _ in range(middle_blk_num)]
            )
...
if __name__ == '__main__':
    ...
    net = SAGNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
