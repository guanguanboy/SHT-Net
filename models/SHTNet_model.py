...
class SHTNetModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
       
        parser.set_defaults(norm='instance', netG='SHTNet', dataset_mode='ihd')
        if is_train:
    ...
    def forward(self):
        ...
        self.harmonized_small, self.harmonized = self.netG(inputs=self.inputs)
        if not self.isTrain:
            self.harmonized = self.comp*(1-self.mask) + self.harmonized*self.mask
            self.harmonized_small = self.comp_small *(1-self.mask_small) + self.harmonized_small*self.mask_small
    ...
    def compute_G_loss(self):
        ...
        self.loss_G_L1_small = self.criterionL1(self.harmonized_small, self.real_small)*self.opt.lambda_L1
        ...
        self.loss_G = self.loss_G_L1 + self.loss_G_L1_small
