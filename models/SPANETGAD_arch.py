...
class SPANetGadient(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.conv_mid = nn.Conv2d(embed_dim*2, embed_dim, kernel_size=3, padding=1, bias=True)

        self.ending = nn.Conv2d(in_channels=embed_dim, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)

        #gradient branch
        self.get_gradient = GetGradientNopadding()
        self.conv_in = nn.Conv2d(3, embed_dim, kernel_size=3, padding=1, bias=True)

        self.grad_out = nn.Conv2d(embed_dim, 3, kernel_size=1, padding=0, bias=True)

        self.act = nn.LeakyReLU(0.1, True)

        self.aff_final = AFF(embed_dim, self.act)
        self.b_concat_1 = nn.Conv2d(2 * embed_dim, embed_dim, kernel_size=3, padding=1, bias=True)
        self.b_block_1 = RCB(2 * embed_dim, self.act, bias=True)

        self.b_concat_2 = nn.Conv2d(3 * embed_dim, embed_dim, kernel_size=3, padding=1, bias=True)
        self.b_block_2 = RCB(3 * embed_dim, self.act, bias=True)
    ...
    def forward(self, x, mask=None):
        ...
        x_cat_1 = torch.cat([x_b_fea, y], dim=1)

        x_cat_1 = self.b_block_1(x_cat_1)
        x_cat_1 = self.b_concat_1(x_cat_1)

        x_cat_2 = torch.cat([x_cat_1, deconv3], dim=1)
        ...
        # Output Projection
        output = self.ending(out)

        #output = y + x[:,:3,:,:]

        return output, res_grad
...
if __name__ == "__main__":
    ...
    model_restoration = SPANetGadient(img_size=input_size, in_chans=3, dd_in=4, embed_dim=32,depths=depths,
    ...
    #macs, params = get_model_complexity_info(model_restoration, (4, input_size, input_size), as_strings=True,
    ...
    output, gradient = model_restoration(x)
    print(output.shape)
    """
