...
from models.SPANETGAD_arch import GetGradientNopadding

class SPANETGADModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
       
        parser.set_defaults(norm='instance', netG='SPANETGAD', dataset_mode='ihd')
        if is_train:
    ...
    def set_input(self, input):
        ...
        #self.mask_small = F.interpolate(self.mask, size=[256,256])
        #self.revert_mask = 1-self.mask
        #self.comp_small = F.interpolate(self.comp, size=[256,256])
        #self.real_small = F.interpolate(self.real, size=[256,256])
    ...
    def forward(self):
        ...
        self.harmonized, self.gradient = self.netG(inputs=self.inputs)
        if not self.isTrain:
    ...
    def compute_G_loss(self):
        ...
        self.loss_G_L1_gadient = self.criterionL1(get_grad(self.harmonized), get_grad(self.real)) + self.criterionL1(self.gradient, get_grad(self.real))
        #self.loss_G_L1_gadient = self.criterionL1(self.gradient, self.real_small)*self.opt.lambda_L1
        ...
        self.loss_G = self.loss_G_L1 + self.loss_G_L1_gadient*self.opt.lambda_L1
