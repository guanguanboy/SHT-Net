...
class SimplifiedFeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, drop_out_rate=0.):
        super(SimplifiedFeedForward, self).__init__()
...
class SPATransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
                 ...
                 norm_layer=nn.LayerNorm,
                 #如下三个参数来自NAFNet
                 DW_Expand=2, FFN_Expand=1, drop_out_rate=0.):
        ...
        self.naf_ffn = SimplifiedFeedForward(dim=dim, ffn_expansion_factor=FFN_Expand)
...
class BasicSPANetLayerNew(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        window_size = win_size
        self.blocks = nn.ModuleList([
            ScaleAwareGatedBlock(c=dim) for i in range(depth)
        ])
...
class SPANet(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # Decoder
        self.upsample_0 = nn.ConvTranspose2d(in_channels=embed_dim*16, out_channels=embed_dim*8, kernel_size=4, stride=2, padding=1)
