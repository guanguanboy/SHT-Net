...
class SPANet(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.intro = nn.Conv2d(in_channels=dd_in, out_channels=embed_dim, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        self.input_proj = InputProj(in_channel=dd_in, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        ...
        self.reduce_chanel = nn.Conv2d(in_channels=embed_dim*2, out_channels=embed_dim, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
    ...
    def forward(self, x, mask=None):
        ...
        #add shallow and deep feature 
        deep_feats = self.reduce_chanel(deconv3)
        combined_feats = y + deep_feats
        # Output Projection
        harmonized = self.ending(combined_feats)
        output = harmonized + x[:,:3,:,:]
        return output
