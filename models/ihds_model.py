...
class ihdsModel(BaseModel):
    ...
    def forward(self):
        ...
        self.harmonized, self.harmonized_lowres = self.netG(inputs=self.inputs)
        if not self.isTrain:
    ...
    def compute_G_loss(self):
        ...
        #将self.real 使用bilinear插值进行4倍的下采样
        self.real_lowres = torch.nn.functional.interpolate(self.real, scale_factor=0.25, mode='bilinear')

        self.loss_G_L1 = self.criterionL1(self.harmonized, self.real) + self.criterionL1(self.harmonized_lowres, self.real_lowres)
