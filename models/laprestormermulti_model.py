...
#from lap_pyr_model import Lap_Pyramid_Conv #for debug

class laprestormermultiModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
       
        parser.set_defaults(norm='instance', netG='LAPRESTORMERMULTI', dataset_mode='ihd')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.decompostion_level = 2
        self.lap_pyramid = Lap_Pyramid_Conv(num_high=self.decompostion_level, device=torch.device('cuda'))
    ...
    def forward(self):
        ...
        self.harmonized, self.restored_low = self.netG(inputs=self.inputs)
        if not self.isTrain: #如果是推理直接走这个分支
            self.harmonized = self.comp*(1-self.mask) + self.harmonized*self.mask


    def compute_G_loss(self):
        """Calculate GAN and L1 loss for the generator"""
        real_pyr = self.lap_pyramid.pyramid_decom(self.real)
        real_low = real_pyr[-1]
        
        loss_weight = self.decompostion_level * self.decompostion_level
        self.loss_G_L1 = self.criterionL1(self.harmonized, self.real) + loss_weight * self.criterionL1(self.restored_low, real_low)
