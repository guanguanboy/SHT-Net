...
class GobalTransformerAttentionLayer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=0,
        ...
        """
        super(GobalTransformerAttentionLayer, self).__init__()
        ...
        self.conv_first = nn.Conv2d(in_c, embed_dim, 3, 1, 1)

        self.conv = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)

        self.conv_last = nn.Conv2d(embed_dim, in_c, 3, 1, 1)
    ...
    def forward_features(self, x):
        ...
        print('after transformer blocks shape==', x.shape)
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        short_cut = x
        x = self.residual_group(x, x_size) #1,4096,24
        x = self.patch_unembed(x, x_size) #1,24,64,64
        x = self.conv(x)#1,24,64,64
        x = self.patch_embed(x)#1,4096,24
        return short_cut + x
...
class LapSwinIR(nn.Module):
    ...
    def forward_features_refinement(self, x):
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_last(res) #1,24, 64, 64
...
def testGobalTransformerAttentionLayer():
    model = GobalTransformerAttentionLayer(img_size=256,
                              patch_size=4,
                              embed_dim=8,
                              depth=2,
                              num_heads=8)
...
if __name__ == '__main__':
    ...
    height = 1024 
    width = 1024
    model = LapSwinIR(upscale=1, in_chans=4, img_size=(1024, 1024),
                   window_size=window_size, img_range=1., depths=[1, 1, 1, 1],
                   embed_dim=24, num_heads=[1, 1, 1, 1], mlp_ratio=2, upsampler='').cuda()
    print(model)
    #print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 4, 1024, 1024)).cuda()
    x = model(x)
    print(x.shape)
    
    #testGobalTransformerAttentionLayer()
